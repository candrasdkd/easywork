...
export default function ProfilePage() {
        ...
        const unsub = onAuthStateChanged(auth, async (u) => {
            ...
            if (u) {
                ...
                try {
                    ...
                    setPicName((snap.data() as any)?.pic_name ?? '');
                } catch (err: any) {
    ...
    const handleSavePicName = async () => {
        ...
        try {
            ...
            await setDoc(
                ref,
                { uuid_account: user.uid, pic_name: picName?.trim() ?? '' },
                { merge: true }
            );
            setSavedOk(true);
            ...
            >
                ...
                >
                    <Stack direction="row" spacing={2} alignItems="center">
                        ...
                        <Box sx={{ minWidth: 0 }}>
                            ...
                            >
                                ...
                                <Chip size="small" label={provider?.providerId ?? 'firebase'} sx={{ ml: 0.5 }} />
                            </Stack>
                        </Box>
                    </Stack>
                    ...
                    <Button
                        ...
                        sx={{
                            borderRadius: 2,
                            alignSelf: { xs: 'stretch', sm: 'center' }, // <-- full width di mobile
                        }}
                    >
                        Logout
                    </Button>
                        ...
                        <Grid size={{ xs: 12, sm: 6 }}>
                            ...
                            >
                                <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
                                    ...
                                    <Stack spacing={1.5}>
                                        <InfoRow label="Nama Lengkap" value={user.displayName ?? 'Pengguna'} />
                                        <InfoRow
                                            label="Dibuat"
                                            value={meta.creationTime ? new Date(meta.creationTime).toLocaleString() : '—'}
                                        />
                                        <InfoRow
                                            label="Terakhir Login"
                                            value={meta.lastSignInTime ? new Date(meta.lastSignInTime).toLocaleString() : '—'}
                                        />
                                    </Stack>
                                </CardContent>
                            </Card>
                                    ...
                                    <Stack spacing={1.5}>
                                        ...
                                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                                            <Button
                                                ...
                                                sx={{
                                                    borderRadius: 2,
                                                    width: { xs: '100%', sm: 'auto' }, // <-- tombol full width di mobile
                                                }}
                                            >
                                                Simpan
                                            </Button>
...
/** Baris info responsif: 1 kolom di mobile, 3 kolom di ≥sm */
function InfoRow({ label, value, copyable }: { label: string; value: string; copyable?: boolean }) {
    ...
    return (
        ...
        >
            ...
            <Typography
                variant="subtitle2"
                sx={{ wordBreak: 'break-word', overflowWrap: 'anywhere' }} // <-- pecah kata panjang
            >
                {value}
            </Typography>
            {copyable && (
                <Button
                    ...
                    sx={{
                        ...
                        width: { xs: '100%', sm: 'auto' }, // <-- button salin full width di mobile
                    }}
                >
